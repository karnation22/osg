...
def return_folder_branch(report_type_date):
    ...
    elif(report_type=='monthly'): return month+"_"+year
...
def find_Jasper_count(report_type_date):
    ...
    url = '''http://{}-{}-{}-{}-{}.compute-{}.amazonaws.com:{}/jasperserver
	     /rest_v2/resources?type=file&folderURI=LatestJLIPDFs/'''.format('ec2',52,45,228,251,1,9090,,)+folder
    print('URL: %s'%url)
    try: 
	req = requests.get(url=url, auth=HTTPBasicAuth('steve.fan@optimalstrategix.com', 1234))
	print('status code: %s'%req.status_code)
    except: 
	print("unable to connect to JASPERSOFT for given run date, url, and credentials")
